...
const debug = false;
// twitch-rewards bot: pronomes customizados cadastrados em APP_HOST
const rewardsServer = "https://bazzite.tail6360a1.ts.net";
const pronounsCache = new Map();

async function getBotPronouns(displayName) {
    // Retorna pronomes do bot (prioridade) ou null se o usuário não cadastrou
    if (pronounsCache.has(displayName)) {
        return pronounsCache.get(displayName);
    }
    try {
        const response = await fetch(`${rewardsServer}/users/${encodeURIComponent(displayName)}`);
        if (response.status != 200) {
            pronounsCache.set(displayName, null);
            return null;
        }
        const data = await response.json();
        const pronouns = data.pronouns || null;
        pronounsCache.set(displayName, pronouns);
        return pronouns;
    } catch (err) {
        console.log(`[twitch-rewards] bot offline, usando pronouns do Firebot (${err.message})`);
        pronounsCache.set(displayName, null);
        return null;
    }
}
...
async function add_chat_msg(chat_msg) {
    ...
    // Linha de cima: somente (Pronomes) em destaque — se houver
    let pronoun_line = document.createElement('div');
    pronoun_line.className = 'pronoun_line';
    // Add pronouns: prioriza pronomes do bot twitch-rewards; cai pro Firebot se não houver
    const botPronouns = await getBotPronouns(chat_msg['display-name']);
    const finalPronouns = botPronouns || chat_msg.pronouns;
    if (finalPronouns) {
        let pronoun_p = document.createElement('p');
        pronoun_p.className = 'pronoun_tag';
        let pronoun_text = document.createTextNode(`(${finalPronouns})`);
        pronoun_p.appendChild(pronoun_text);
        pronoun_line.appendChild(pronoun_p);
        user_div.appendChild(pronoun_line);
    }
    // Linha com Nome: texto
    let line_div = document.createElement('div');
    line_div.className = 'name_pronoun_line';
    // Add the Username
    ...
    line_div.appendChild(name_p);
    // Dois pontos + texto na mesma linha
    let colon_span = document.createElement('span');
    colon_span.className = 'msg_colon';
    colon_span.textContent = ': ';
    line_div.appendChild(colon_span);
    user_div.appendChild(line_div);
    // Main message text
    ...
    // Texto entra na mesma linha do Nome (Pronomes):
